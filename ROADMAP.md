...
- **プロセス演習(`myrun`)進行中(2026-09-09 着手)** — Step 1(`fork`/`exec`/`waitpid`)、
  Step 2(`pipe`)、Step 3(終了ステータスの分解 / シグナル死)まで完了(2026-09-14)。
  記録は `docs/notes/phase-minus1-process.md`。次は **Step 4(タイムアウトで子を `kill`)** から
...
- [ ] 押さえる概念:
    ...
    - [ ] プロセスとfork/exec、シグナル、パイプ — `myrun` で演習中(全5ステップ)。Step 1〜3 完了、次は Step 4(タイムアウトで `kill`)(`docs/notes/phase-minus1-process.md`)
