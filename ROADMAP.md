...
- **プロセス演習(`myrun`)は Step 1〜5 完了(2026-09-15)** — fork/exec/pipe/シグナル死/タイムアウト(SIGTERM→SIGKILL、
  プロセスグループで孫まで)が動き、`strace -f` で全体を読んだ。残りは末尾の **「自分の言葉で書く」2枠**のみ。
...
- [ ] 押さえる概念:
    ...
    - [ ] プロセスとfork/exec、シグナル、パイプ — `myrun` で演習中(全5ステップ)。Step 1〜5 完了、残りは「自分の言葉で書く」2枠(`docs/notes/phase-minus1-process.md`)
