...
- **Phase -1(OSの基礎)完了(2026-09-24)** — 本を読みながら `experiments/oslearn/` で `nix` を使った演習を3本(`mycp` / `myrun` / `mymem`)やり切った。
  次は Phase 0 の「用語の地図」
...
- **仮想メモリ演習(`mymem`)は完了(2026-09-24)** — Phase -1 の最後の項目。全5ステップ
  ...
  **Step 4 完了(2026-09-23)** — `/proc/self/pagemap` で仮想→物理(PFN)を自分で引いた。ゼロページは
  システム全体で1枚(PFN 固定)、`fork` 後は親子が同じ PFN → 子が書くと子の PFN だけ変わる(CoW の直接証拠)。
  **Step 5 完了(2026-09-24)** — `userfaultfd` でページフォルトを自分のコードで処理した。5-1 でページを自分で埋め、
  5-2 でスナップショット復元のまねごと(200 ページ中、触った 20 ページだけファイルから読む。`strace` で確認)。
  用語集に「## 仮想メモリ」12語を追加し、「なぜ仮想アドレスという間接層があるのか」「ページテーブルと EPT」を自分の言葉で書いた。
...
## Phase -1: OSの基礎(2〜3週)【完了 2026-09-24】

KVMを触る前に、ユーザー空間とカーネルの境界を体で理解しておく。ここが曖昧なままPhase 3に入るとほぼ確実に足が止まる。

- [x] 『ふつうのLinuxプログラミング』を入手して読み始める
- [x] 読み終える。**読み終えてから始めるのではなく、読みながら手を動かす**
- [x] 本のサンプルはCだが、**Rustと`nix`クレートで書き直しながら読む**。CとRustの差分が理解のフックになる
- [x] `strace` で自分のプログラムが発行したシステムコールを観察する習慣をつける
- [x] 押さえる概念:
    ...
    - [x] プロセスとfork/exec、シグナル、パイプ — `myrun` 演習 完了(全5ステップ、2026-09-15)(`docs/notes/phase-minus1-process.md`)
    - [x] 仮想メモリ(こちらも「仮想化」と呼ばれるが別概念。ただしEPTの理解に直結する) — `mymem` 演習 完了(全5ステップ、2026-09-24)(`docs/notes/phase-minus1-memory.md`)

### 到達点の確認

- [x] `std::fs::copy` を使わずにファイルコピーを書き、`strace` で発行されたシステムコールを説明できる
- [x] 「なぜアプリはディスクを直接触れないのか」を自分の言葉で説明できる
- [x] `BufReader` が存在する理由を、システムコールのコストの観点で説明できる
...
- [x] リポジトリ作成、名前を決める
- [x] ディレクトリ構成(下記「リポジトリ構成」参照)
- [x] `CLAUDE.md` / `AGENTS.md` を配置
- [ ] CI(後述)
- [x] README
