...
```
775c23b5c000-775c23b5d000    4 KiB  rw-p  /usr/lib/.../libgcc_s.so.1
775c23b62000-775c23b63000    4 KiB  ---p  [匿名]          ← これ
775c23b63000-775c23b65000    8 KiB  rw-p  [匿名]
775c23b65000-775c23b67000    8 KiB  rw-p  [匿名]
```

考える3点:

1. サイズが 4 KiB。これは**何の単位ちょうど**か
2. `---p` の**直後が `rw-p`**。読み書きできる領域のすぐ隣にわざわざ置いてある
3. その `rw-p` を使うコードが、うっかり**範囲を超えて**書き込んだ。`---p` に踏み込んだら何が起きるか

> ヒント: 3 は `myrun` Step 3 で自分で発生させて終了コード **139** を見ている。
...
- [ ] Step 1: 自分のアドレス空間を印字する 🔶 進行中
      ...
      - [ ] 罠B(`---p`) ← 次回ここから。材料はノートに残してある
      - [ ] 罠A(`[heap]` を作る syscall) / 罠C(2回動かすと変わるもの)
      - [ ] 「分かったこと」を自分の言葉で書く
